...
def parse_args():
    ap = argparse.ArgumentParser(description="Evaluate TWO-ring combinations (pairs), no fixed ring.")
    ...
    # OPTIONAL: if set, only evaluate pairs whose sensor count sum == k
    ap.add_argument("--k", type=int, default=None,
                    help="Optional: only evaluate ring pairs with total sensor count == k. If omitted, evaluate all pairs.")
    ...
    ap.add_argument("--out-dir", default="ur_sensor_sim/tmp/ring_experiment_two_rings",
...
def main():
    ...
    # 6) Evaluate all ring pairs
    n = len(ring_names)
    want_k = args.k
    total_pairs = n * (n - 1) // 2
    print(f"[INFO] Evaluating TWO-ring pairs: {n} rings -> ~{total_pairs} pairs"
          + (f" with total size == k={want_k}" if want_k is not None else ""))

    best = None   # (obj, ring_a, ring_b, total_size)
    topN = []
    checked = 0
    skipped_k = 0
    ...
    for i in range(n):
        ...
        for j in range(i + 1, n):
            b = ring_names[j]
            total_size = size_a + ring_sizes[b]

            if want_k is not None and total_size != want_k:
                skipped_k += 1
                continue

            covered = cov_a | ring_cov[b]
            ...
            checked += 1
            if best is None or obj > best[0]:
                best = (obj, a, b, total_size)
                elapsed = time.time() - t0
                print(f"[BEST] obj={obj:.3f} | rings={a}, {b} | size={total_size} | checked={checked} | elapsed={elapsed:.1f}s")

            _push_top({
                "objective_value": float(obj),
                "rings": [a, b],
                "total_size": int(total_size),
                "indices": sorted(set(rings_abs[a] + rings_abs[b])),
            })
    ...
    if best is None:
        raise SystemExit("[ERROR] No pairs evaluated (maybe your --k filtered everything?).")

    best_obj, ra, rb, best_size = best
    ...
    best_json = {
        ...
        "rings": [ra, rb],
        "total_size": int(best_size),
        "indices": sorted(set(rings_abs[ra] + rings_abs[rb])),
        "S": int(S), "V": int(V), "P": int(P),
        "heatmap_sources": heatmap_files,
        "weights_sources": weight_files,
        "normalize_weights": bool(args.normalize_weights),
        "k_filter": (None if want_k is None else int(want_k)),
        "notes": "Two-ring (pair) evaluation only (no fixed ring).",
    }
    (out_dir / "best_2rings.json").write_text(json.dumps(best_json, indent=2))
    (out_dir / "top_2rings.json").write_text(json.dumps(topN, indent=2))

    with open(out_dir / "top_2rings.csv", "w") as f:
        f.write("rank,objective_value,ring_a,ring_b,total_size\n")
        for r, entry in enumerate(topN, start=1):
            a, b = entry["rings"]
            f.write(f"{r},{entry['objective_value']:.10f},{a},{b},{entry['total_size']}\n")

    elapsed = time.time() - t0
    print("\n========== DONE ==========")
    print(f"[OK] Best: obj={best_obj:.3f} | rings={ra}, {rb} | size={best_size}")
    print(f"[OK] Evaluated={checked} pairs (skipped_k={skipped_k}) | elapsed={elapsed:.1f}s")
    print(f"[OK] Wrote: {out_dir / 'best_2rings.json'}")
    print(f"[OK] Wrote: {out_dir / 'top_2rings.json'}")
    print(f"[OK] Wrote: {out_dir / 'top_2rings.csv'}")
