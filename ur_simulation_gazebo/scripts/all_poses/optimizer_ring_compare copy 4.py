...
def parse_args():
    ...
    ap.add_argument("--out-dir", default="ur_sensor_sim/tmp/ring_experiment_flange_fixed_seperate_p_3to5",
    ...
    ap.add_argument("--poses", nargs="*", default=["b3_w1","b3_w2","b3_w3","b4_w1","b4_w2","b4_w3","b5_w1","b5_w2","b5_w3"],
                help="Optional list of pose names to run (e.g. b3 b4 b5).")
...
def main():
    ...
    pose_names = [Path(f).stem for f in weight_files]
    if args.poses:
        want = set(args.poses)
        pose_indices = [i for i, nm in enumerate(pose_names) if nm in want]
        missing = sorted(want - set(pose_names))
        if missing:
            print(f"[WARN] requested poses not found in weights files: {missing}")
    else:
        pose_indices = list(range(P))

    pose_indices = sorted(pose_indices)
    if not pose_indices:
        raise SystemExit("[ERROR] No poses selected (pose_indices empty).")

    pose_names_sel = [pose_names[i] for i in pose_indices]
    W_sel = W[:, pose_indices]          # (V, Psel)
    Psel = len(pose_indices)
    print(f"[INFO] Using {Psel} poses: {pose_names_sel}")
    ...
    print("[INFO] Precomputing per-ring coverage masks (selected poses only) ...")
    ring_cov = {}  # name -> (Psel, V) bool

    for name in ring_names:
        cov = np.zeros((Psel, V), dtype=bool)
        for sidx in rings_abs[name]:
            for pp, p in enumerate(pose_indices):   # pp = 0..Psel-1, p = original pose index
                idx = rows[p][sidx]
                if idx.size:
                    cov[pp, idx] = True
        ring_cov[name] = cov

    # 6) Exhaustive search PER POSE: fixed ring + all pairs of remaining rings
    ...
    n_other = len(others)
    total_pairs = n_other * (n_other - 1) // 2

    print(f"[INFO] Per-pose evaluation: fixed='{fixed_name}' + pairs from {n_other} other rings "
          f"(~{total_pairs} pairs per pose), k={args.k}, P={P}")

    t0_all = time.time()

    for pp, p in enumerate(pose_indices):
        pose_label = pose_names[p]
        pose_dir = out_dir / pose_label
        pose_dir.mkdir(parents=True, exist_ok=True)

        # Slice to single-pose view
        Wp = W_sel[:, [pp]]                          # (V,1)
        fixed_cov_p = ring_cov[fixed_name][[pp], :]  # (1,V)
        ...
        print(f"\n[POSE {pp+1}/{Psel}] {pose_label}: evaluating ~{total_pairs} combos ...")

        t0 = time.time()
        for i in range(n_other):
            b = others[i]
            cov_b_p = ring_cov[b][[pp], :]               # (1,V)
            ...
            for j in range(i + 1, n_other):
                ...
                covered_p = fixed_cov_p | cov_b_p | ring_cov[c][[pp], :] # (1,V)
                ...
                if (best is None) or (obj > best[0]):
                    ...
                    elapsed = time.time() - t0
                    print(f"[POSE BEST] obj={obj:.3f} | rings={fixed_name}, {b}, {c} | fixed={fixed_count} "
        ...
        best_json = {
            ...
            "S": int(S), "V": int(V), "P_used": 1,
            "heatmap_sources": heatmap_files,
            "weights_source": weight_files[p] if p < len(weight_files) else None,
            "normalize_weights": bool(args.normalize_weights),
            "notes": "Per-pose exhaustive search: fixed ring + any other two rings.",
        }
        (pose_dir / "best_fixedplus2.json").write_text(json.dumps(best_json, indent=2))
        (pose_dir / "top_fixedplus2.json").write_text(json.dumps(topN, indent=2))

        with open(pose_dir / "top_fixedplus2.csv", "w") as f:
        ...
        print(f"[POSE DONE] {pose_label}: checked={checked} (skipped_k={skipped_k}) "
              f"| best_obj={best_obj:.3f} | elapsed={elapsed_pose:.1f}s")
        print(f"[POSE OUT] {pose_dir}")

    elapsed_all = time.time() - t0_all
    print("\n========== ALL POSES DONE ==========")
    print(f"[OK] Wrote per-pose outputs into: {out_dir}")
    print(f"[OK] Total elapsed: {elapsed_all:.1f}s")


    # ---- TOTAL over selected poses ----
    fixed_cov = ring_cov[fixed_name]  # (Psel,V)
    ...
    def _push_top_total(entry):
        topN_total.append(entry)
        topN_total.sort(key=lambda x: x["objective_value"], reverse=True)
        if len(topN_total) > int(args.top_n):
            topN_total.pop()
    ...
    for i in range(n_other):
        ...
        for j in range(i + 1, n_other):
            ...
            covered = fixed_cov | ring_cov[b] | ring_cov[c]   # (Psel,V)
            obj = objective_from_masks(covered, W_sel, mode=args.objective,
                                    softmin_temp=args.softmin_temp, frac_alpha=args.frac_alpha)

            checked_total += 1
            if (best_total is None) or (obj > best_total[0]):
                best_total = (obj, (fixed_name, b, c), fixed_count)
    ...
    # write total outputs into out_dir root
    best_obj, (ra, rb, rc), fixed_count = best_total
    (out_dir / "best_total_selectedposes.json").write_text(json.dumps({
        "status": "ok",
        "poses_used": pose_names_sel,
        "objective_mode": args.objective,
        "objective_value": float(best_obj),
        "rings": [ra, rb, rc],
        "k": int(args.k),
        "fixed_count": int(fixed_count),
        "fixed_indices": sorted(set(rings_abs[ra] + rings_abs[rb] + rings_abs[rc])),
        "P_used": int(Psel),
        "notes": "TOTAL over selected poses (e.g. b3/b4/b5 set).",
    }, indent=2))

    (out_dir / "top_total_selectedposes.json").write_text(json.dumps(topN_total, indent=2))

    with open(out_dir / "top_total_selectedposes.csv", "w") as f:
        f.write("rank,objective_value,fixed_ring,ring_b,ring_c,fixed_count\n")
        for r, entry in enumerate(topN_total, start=1):
            fr, b, c = entry["rings"]
            f.write(f"{r},{entry['objective_value']:.10f},{fr},{b},{c},{entry['fixed_count']}\n")
