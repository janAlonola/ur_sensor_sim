...
def process_pose(pose_name: str):
    ...
    # Visibility for these sensors (new-only in append mode)
    ...
    # ---------------- APPEND MODE ----------------
    if GLOBAL_APPEND_DIR is not None:
        old_path = GLOBAL_APPEND_DIR / f"{GLOBAL_VOXEL_STEM}_{pose_name}_heatmap.yaml"
        if not old_path.is_file():
            raise FileNotFoundError(f"Append requested but missing old heatmap: {old_path}")
        print("started")
        old_doc = load_yaml(old_path)
        print("old doc loaded")
        if int(old_doc["voxel_count"]) != len(GLOBAL_VOXELS):
    ...
    # ---------------- FRESH MODE ----------------
        ...
        coverage = visible.sum(axis=1).astype(int).tolist()
        visible_by = [np.nonzero(visible[i])[0].tolist()
                      for i in range(len(GLOBAL_VOXELS))]
    ...
    coverage_arr = np.asarray(out_doc["coverage"], dtype=int)
    unseen = int((coverage_arr == 0).sum())
    mean_cov = float(coverage_arr.mean())
...
def main():
    ...
    ap.add_argument("--candidates", default="ur_sensor_sim/mesh_sampling/big_candidates_with_ring.yaml",
    ...
    ap.add_argument("--out", default="ur_sensor_sim/tmp/big_visibility_ring",
    ...
    indices = [2658, 2659, 2660, 2661, 2662, 2663, 2664]
