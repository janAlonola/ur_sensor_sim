...
def parse_args():
    ...
    ap.add_argument("--k", type=int, default=32, help="Total sensor budget (must equal total sensors in the chosen 3 rings).")
    ...
    ap.add_argument("--out-dir", default="ur_sensor_sim/tmp/ring_experiment_flange_fixed_4_rings",
...
def main():
    ...
    print(f"[INFO] Evaluating all (fixed + 3)-ring combos: fixed='{fixed_name}' "
        f"+ triples from {n} other rings (~{total_triples} combos), keeping k={args.k} ...")
    ...
    for i in range(n):
        ...
        for j in range(i + 1, n):
            ...
            for k3 in range(j + 1, n):
                ...
                fixed_count = fixed_size + ring_sizes[b] + ring_sizes[c] + ring_sizes[d]
                if fixed_count != args.k:
                    skipped_k += 1
                    continue

                covered = fixed_cov | ring_cov[b] | ring_cov[c] | ring_cov[d]
                ...
                checked += 1
                if (best is None) or (obj > best[0]):
                    best = (obj, (fixed_name, b, c, d), fixed_count)
                    elapsed = time.time() - t0
                    print(f"\n[BEST] obj={obj:.3f} | rings={fixed_name}, {b}, {c}, {d} | fixed={fixed_count} "
                        f"| checked={checked} | elapsed={elapsed:.1f}s")

                _push_top({
                    "objective_value": float(obj),
                    "rings": [fixed_name, b, c, d],
                    "fixed_count": int(fixed_count),
                    "fixed_indices": sorted(set(fixed_abs + rings_abs[b] + rings_abs[c] + rings_abs[d])),
                })
    ...
    best_obj, (ra, rb, rc, rd), fixed_count = best
    best_fixed = sorted(set(rings_abs[ra] + rings_abs[rb] + rings_abs[rc] + rings_abs[rd]))
    ...
    with open(out_dir / "top_fixedplus3.csv", "w") as f:
        f.write("rank,objective_value,fixed_ring,ring_b,ring_c,ring_d,fixed_count\n")
        for r, entry in enumerate(topN, start=1):
            fixed_ring, b, c, d = entry["rings"]
            f.write(f"{r},{entry['objective_value']:.10f},{fixed_ring},{b},{c},{d},{entry['fixed_count']}\n")
    ...
    print(f"[OK] Wrote: {out_dir / 'best_4rings.json'}")
    print(f"[OK] Wrote: {out_dir / 'top_4rings.csv'}")
    print(f"[OK] Wrote: {out_dir / 'top_4rings.json'}")
